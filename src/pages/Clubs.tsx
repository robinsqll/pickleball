...
type Level = NonNullable<ClubCardProps['level']>;
...
export default function Clubs() {
  ...
  // on ignore la valeur courante, seul le setter est utile
  const [, setViewMode] = useState<'list' | 'map'>('list');

  /* ---------- Données dérivées (mémorisées) ---------- */

  const regions = useMemo(() => {
    const unique = new Set<Region>();
    clubs.forEach((club) => unique.add(club.location.region));
  ...
  const levels = useMemo(() => {
    ...
    clubs.forEach((club) => {
      if (club.level) unique.add(club.level);
    });
    return Array.from(unique).sort();
  }, []);
  ...
  const filteredClubs = useMemo(() => {
    return clubs.filter((club) => {
      ...
      const matchesRegion =
        selectedRegion === 'all' || club.location.region === selectedRegion;

      const matchesLevel =
        selectedLevel === 'all' || club.level === selectedLevel;
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-16">
            ...
            {(selectedRegion !== 'all' ||
              ...
              <div className="flex items-center gap-2 my-6">
                ...
                <p className="text-sm text-muted-foreground">
                  ...
                  {searchQuery && (
                    <span className="ml-1">
                      {selectedRegion !== 'all' || selectedLevel !== 'all'
                        ? ' | '
                        : ''}
        ...
        {/* ---------- Bannière “Créer un club” ---------- */}
        <div className="mt-16 p-6 bg-muted rounded-lg">
          <div className="max-w-3xl mx-auto">
            <h3 className="text-xl font-medium mb-2 text-center">
              Vous souhaitez créer un club?
            </h3>
            <p className="text-muted-foreground text-center mb-6">
              Nous vous accompagnons dans la création de votre club de
              pickleball. Contactez‑nous pour obtenir plus d'informations et des
              ressources.
            </p>
